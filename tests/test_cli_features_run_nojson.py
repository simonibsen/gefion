...
def test_features_run_no_symbols_errors_plaintext(monkeypatch):
    # No stocks, no exchange, no listings -> should emit error and non-zero exit
    class DummyClient:
...
def test_features_run_infers_symbols_plaintext(monkeypatch):
    conn = require_db()
    conn.autocommit = True
