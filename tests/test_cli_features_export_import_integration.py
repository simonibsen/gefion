"""
Integration tests for the improved features-export and features-import CLI commands.
"""
import json
import os
import psycopg
...
def test_export_with_default_directory(setup_test_data, tmp_path, monkeypatch):
    """Test that features-export uses default 'feature-functions' directory."""
    monkeypatch.chdir(tmp_path)

    result = runner.invoke(cli.app, ["features-export"])
...
def test_export_with_custom_directory(setup_test_data, tmp_path):
    """Test that features-export works with custom directory."""
    export_dir = tmp_path / "custom-export"

    result = runner.invoke(cli.app, ["features-export", "--dir", str(export_dir)])
...
def test_export_filtered_functions(setup_test_data, tmp_path, monkeypatch):
    ...
    result = runner.invoke(cli.app, ["features-export", "--functions", "cli_test_func1"])
...
def test_import_with_default_directory(db_conn, tmp_path, monkeypatch):
    ...
    result = runner.invoke(cli.app, ["features-import"])
...
def test_import_with_custom_directory(db_conn, tmp_path):
    ...
    result = runner.invoke(cli.app, ["features-import", "--dir", str(import_dir)])
...
def test_import_filtered_functions(db_conn, tmp_path, monkeypatch):
    ...
    # Import only func1
    result = runner.invoke(cli.app, ["features-import", "--functions", "cli_test_import1"])
...
def test_import_is_idempotent_via_cli(db_conn, tmp_path, monkeypatch):
    ...
    # First import
    result1 = runner.invoke(cli.app, ["features-import"])
    assert result1.exit_code == 0
    assert "Imported 1" in result1.stdout

    # Second import - should succeed
    result2 = runner.invoke(cli.app, ["features-import"])
...
def test_export_import_roundtrip(setup_test_data, db_conn, tmp_path, monkeypatch):
    ...
    # Export
    result_export = runner.invoke(cli.app, ["features-export"])
    ...
    # Import
    result_import = runner.invoke(cli.app, ["features-import"])
...
def test_import_from_nonexistent_directory(tmp_path, monkeypatch):
    ...
    result = runner.invoke(cli.app, ["features-import", "--dir", "nonexistent"])
