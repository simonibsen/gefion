...
CREATE TABLE IF NOT EXISTS feature_functions (
    ...
    scope TEXT NOT NULL DEFAULT 'stock' CHECK (scope IN ('stock','market','materialized')),  -- 011: per-stock vs market body; 'materialized' = written by its own pipeline, never dispatched
...
ALTER TABLE feature_functions ADD COLUMN IF NOT EXISTS scope TEXT NOT NULL DEFAULT 'stock' CHECK (scope IN ('stock','market','materialized'));
...
-- =============================================================================
-- MODELING UNIVERSE (015, owner-approved 2026-07-19)
-- =============================================================================

-- Named, rule-defined subsets of the stock population — the entity-space
-- sibling of regime_definitions. Rules/pins are generic predicates (JSONB);
-- fingerprint = sha256 of canonical rules+pins for result provenance.
...
-- Membership in COMPLEMENT form: a symbol is a member as-of date D iff no
-- row here covers (universe, symbol, D). Static rules yield one open-ended
-- row per excluded symbol; every row names the rule/pin that caused it.
