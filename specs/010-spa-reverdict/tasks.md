...
- [ ] T022 Learning materials: `.claude/commands/gefion-learn.md` Module 10 aside (BH corrects the p-values you have; SPA models the search that produced them) + checkpoint (why is the drift refusal a feature?); curriculum drift test green
- [ ] T023 Observability pass: `OTEL_ENABLED=true` re-verdict on a dev run; `gefion span-check` — spans parented, no orphans; runtime sanity (B=1000, v1 family in seconds–minutes)
- [ ] T024 Full-suite pre-flight: drop `gefion_test`, complete suite against a fresh DB (exit code captured — the pipe-masking lesson); docs-drift + dictionary `--check` green
