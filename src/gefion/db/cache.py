...
def prefetch_stock_ids(conn: psycopg.Connection, symbols: Sequence[str]) -> Dict[str, int]:
    ...
    """
    with create_span("db.cache.prefetch_stock_ids", input_count=len(symbols)) as span:
        if not symbols:
        ...
        set_attributes(span, result_count=len(result), table="stocks")
        return result
...
def prefetch_latest_prices(
    ...
    """
    with create_span("db.cache.prefetch_latest_prices", input_count=len(stock_ids)) as span:
        if not stock_ids:
            set_attributes(span, result_count=0)
            return {}
        ...
        result = {row[0]: row[1] for row in rows}
        set_attributes(span, result_count=len(result), table="stock_ohlcv")
        return result
...
def prefetch_feature_ids(
    ...
    """
    with create_span("db.cache.prefetch_feature_ids", input_count=len(feature_names)) as span:
        ...
        result = {row[0]: row[1] for row in rows}
        set_attributes(span, result_count=len(result), table="feature_definitions")
        return result
