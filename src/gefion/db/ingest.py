...
def filter_new_rows(
    ...
    Args:
        ...
        since_date: Minimum date to allow (inclusive). Rows with dates before
                   this will be filtered out. Defaults to None (no limit).
    ...
    for row in rows:
        ...
        if parsed_date > latest:
            # Check since_date lower bound
            if since_date is not None and parsed_date < since_date:
                continue
...
def ensure_store_targets(conn: psycopg.Connection, defs: Sequence[Mapping[str, object]]) -> None:
    ...
    for d in defs:
        ...
        if table == "computed_features":
            from gefion.db import schema  # local import to avoid cycles
