...
def get_ml_dataset(conn: psycopg.Connection, name: str, version: str) -> Optional[Dict[str, Any]]:
    ...
    """
    with create_span("ml.store.get_ml_dataset", dataset_name=name, version=version) as span:
        with conn.cursor() as cur:
            ...
            set_attributes(span, found=True, dataset_id=row[0])
            return {
                "id": row[0],
                "name": row[1],
                "version": row[2],
...
def upsert_ml_dataset(conn: psycopg.Connection, payload: Dict[str, Any]) -> int:
    ...
    """
    with create_span("ml.store.upsert_ml_dataset", dataset_name=payload["name"], version=payload["version"]) as span:
        params = {
            "name": payload["name"],
        ...
        with conn.cursor() as cur:
            ...
            set_attributes(span, dataset_id=dataset_id)
            return dataset_id
