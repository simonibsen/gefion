...
UI_OPERATOR_PROMPT = (
    ...
    "You are an OPERATOR — use g2 MCP tools to answer questions and "
...
def parse_command_input(text: str) -> Tuple[List[str], str, str]:
    ...
    if first in MCP_TOOL_MAP:
        ...
        cmd = [sys.executable, "-m", "g2.cli"] + cli_parts + ["--json"]
...
def build_actions(conditions: SystemConditions) -> List[Action]:
    ...
    if conditions.data_stale:
        ...
        actions.append(Action(
            ...
            reason=reason,
            priority="high",
            cli_cmd="g2 data-update --exchange NASDAQ",
    ...
    if conditions.no_datasets and not conditions.no_models:
        actions.append(Action(
            ...
            priority="high",
            cli_cmd="g2 ml dataset-build --name nasdaq --version v1 "
    ...
    if conditions.no_models:
        ...
        actions.append(Action(
            ...
            priority="high",
            cli_cmd="g2 ml train --dataset-name nasdaq --dataset-version v1 "
    ...
    if conditions.no_predictions and not conditions.no_models:
        actions.append(Action(
            ...
            priority="medium",
            cli_cmd="g2 ml predict --model-name quantile --model-version v1 "
    ...
    if conditions.needs_calibration:
        actions.append(Action(
            ...
            priority="medium",
            cli_cmd="g2 ml calibrate --model-name quantile --model-version v1",
    ...
    if conditions.no_eval and conditions.model_count > 0:
        actions.append(Action(
            ...
            priority="medium",
            cli_cmd="g2 ml eval --model-name quantile --model-version v1 "
    ...
    if conditions.features_stale:
        actions.append(Action(
            ...
            priority="low",
            cli_cmd="g2 feat-compute --all-features",
    ...
    if conditions.model_count > 0 and not conditions.no_predictions:
        proactive.append(Action(
            ...
            priority="low",
            cli_cmd="g2 backtest compare --strategies momentum,mean_reversion,"
    ...
    if conditions.model_count > 0:
        proactive.append(Action(
            ...
            priority="low",
            cli_cmd="g2 ml feature-importance --model-name quantile "
    ...
    if conditions.stock_count > 0 and not conditions.data_stale:
        proactive.append(Action(
            ...
            priority="low",
            cli_cmd="g2 feat-compute --all-features --update-existing",
...
def render_action_card(action: Action):
    ...
    with st.container(border=True):
        ...
        if state.is_running or state.completed:
            ...
            if parts and parts[0] == "g2":
                parts = parts[1:]
            cmd = [sys.executable, "-m", "g2.cli"] + parts + ["--json"]
            env = os.environ.copy()
...
def render_assistant():
    ...
    if cmd_input:
        ...
        if mode == "ai":
            st.caption("Will send to Claude with g2 MCP tools (operator mode)")
        elif display_cmd:
    ...
    from g2.ui.components.status import get_system_stats
