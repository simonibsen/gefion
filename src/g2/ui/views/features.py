...
g2 feat-def-import --dir feature-definitions/""", language="bash")
    ...
    try:
        ...
        if definitions:
            ...
            functions = sorted(set(d.get("function_name", "unknown") for d in definitions))

            for func in functions:
                func_defs = [d for d in definitions if d.get("function_name") == func]
                with st.expander(f"**{func}** ({len(func_defs)} definitions)", expanded=True):
                    for defn in func_defs:
                        render_definition_row(defn)

            st.caption(f"Total: {len(definitions)} definitions")
        else:
...
g2 feat-fx-import --dir feature-functions/""", language="bash")
    ...
    try:
        ...
        if functions:
            ...
            st.caption(f"Total: {len(functions)} functions")
        else:
...
def save_definition_to_json(defn: dict) -> tuple[bool, str]:
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "g2.cli", "feat-def-import", "--dir", str(definitions_dir)],
            capture_output=True,
            text=True,
            env=env,
...
def save_function_to_json(func: dict) -> tuple[bool, str]:
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "g2.cli", "feat-fx-import", "--dir", str(functions_dir)],
            capture_output=True,
            text=True,
            env=env,
            cwd=str(project_root),
